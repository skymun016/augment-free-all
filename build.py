...
class DependencyManager:
    ...
    def install_build_dependencies(self) -> bool:
        ...
        # Upgrade pip first
        try:
            # 使用 sys.executable 确保使用正确的Python解释器
            python_cmd = sys.executable
            run_command(f'"{python_cmd}" -m pip install --upgrade pip', timeout=120)
        ...
        for category, deps in self.build_dependencies.items():
            ...
            for dep in deps:
                try:
                    ...
                    python_cmd = sys.executable
                    run_command(f'"{python_cmd}" -m pip install {dep}', timeout=180)

                    # Verify installation
                    package_name = dep.split('>=')[0].split('==')[0]
                    python_cmd = sys.executable
                    result = run_command(f'"{python_cmd}" -m pip show {package_name}', capture_output=True)
...
class PythonPackageBuilder:
    ...
    def build_python_packages(self) -> bool:
        ...
        try:
            ...
            python_cmd = sys.executable
            run_command(f'"{python_cmd}" -m build --wheel --sdist --outdir dist', timeout=300)
